...
# fills the songnames string with appropriate spacing
# Probably need to change this to some kind of tab scheme later
def makesongnames():
    global songnames
    songnames = ""
...
# makes songnames at the beginning
makesongnames()
...
async def play_error(context, error):
    if isinstance(error, commands.CommandInvokeError):
        await context.message.channel.send("Chill out dude, I'm playing something alreay")
...
async def checkquaid(context):
    user = context.message.author
    roles = context.message.author.roles
    for x in roles:
        # print("xname: ", x.name)
        # print("typex :", type(x.name))
        if x.name == 'Quaid':
            return True
        else:
            pass
    await context.message.channel.send("Error you do not have permission, must be a Quaid.")
    return False
...
def writedictcsv(dictionary, name):
