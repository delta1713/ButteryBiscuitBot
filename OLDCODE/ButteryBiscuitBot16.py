...
async def on_message(message):
    ...
    'good bot': ['*blushes* th..thank you senpai uwu', 'awww, stahp :blush:', f'you\'re a pretty good bot too, {auth1}']
    }
...
OLDCALLRESPONSEDICT = {
    ...
    'salmonelle' : 'chicken',
    'bad bot' : ['i\'ve been a bad bot daddy, i need spanking', '*pees on floor* is that better?!?'],
    'good bot': ['*blushes* th..thank you senpai uwu', 'awww, stahp :blush:', f'you\'re a pretty good bot too']
}
...
@BOT.command(pass_context = True,
    aliases=['disconnect', 'Disconnect', 'fuckoffbiscuitbot', 'd', 'fu'])
...
mongisbool = 0
mongiscid = 105187435956166656
mongiscid = 105187435956166656
mongischannel = BOT.get_channel(mongiscid)

@BOT.command(pass_context = True,
    aliases=['m', 'amongus'])
async def mongis(context):
    global mongisbool
    bc = BOT.get_channel(biscuitchannel)
    bl = BOT.get_channel(botlog)
    mongischannel = BOT.get_channel(105187435956166656)
    mongislist = []
    if mongisbool == 0:
        mongisbool = 1
        for x in mongischannel.members:
            await x.edit(deafen=True)
            if x.nick != None:
                mongislist.append(x.nick)
            else:
                mongislist.append(x.name)
        mongisstr = ", "
        mongisstr = mongisstr.join(mongislist)
        await bc.send(f"{str(context.message.author)} deafened the following members in mongis:\n{mongisstr}")
        await bl.send(f"{str(context.message.author)} deafened the following members in mongis:\n{mongisstr}")
    elif mongisbool == 1:
        mongisbool = 0
        for x in mongischannel.members:
        ...
        mongisstr = ", "
        mongisstr = mongisstr.join(mongislist)
        await bc.send(f"{str(context.message.author)} undeafened the following members in mongis:\n{mongisstr}")
        await bl.send(f"{str(context.message.author)} undeafened the following members in mongis:\n{mongisstr}")
    await context.message.delete()
...
async def printcontext(context):
    ...
    print("context.message.channel is: ", context.message.channel)
    print("context.message.author is: ", context.message.author)
    print("context.message.author.voice is: ", context.message.author.voice)
    print(dir(context))
    print("test")

#@printcontext.error
#async def printcontext_error(context, error):
#    if isinstance(error, commands.CommandError):
...
async def troubleshoot(context):
    ...
    print("context.message.author is: ", context.message.author)
    print("context.message.author.voice is: ", context.message.author.voice)
    print("context.message.author.voice.channel.members is: ", context.message.author.voice.channel.members)
    for x in BOT.voice_clients:
        print("connected voice client: ", BOT.voice_clients)
        #await context.message.channel.send(str(x.channel))
    #await context.message.channel.send(str(context.message))
    print("mongischannel: ", mongischannel)
    mongistest = context.message.author.voice.channel.id
    await context.message.channel.send(f"mongistest: {mongistest}")
...
async def hyd(channel):
    """
    biscuitfaces
    """
    await channel.send(file=discord.File('/pythbot/Buttery_Biscuit_Bot/pictures/hyd.jpg'))
